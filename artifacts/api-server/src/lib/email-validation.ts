...
async function checkMxRecords(domain: string): Promise<EmailValidationResult> {
  ...
  try {
    ...
    if (records && records.length > 0) {
      ...
      // Some domains accept mail at the A record if no MX is set; treat as risky.
      try {
    ...
    if (code === "ENOTFOUND" || code === "ENODATA") {
      // No MX records — many resolvers raise ENODATA in this case rather than
      // returning an empty array. Fall back to A-record check so domains that
      // accept mail at the apex are surfaced as "risky" rather than "invalid".
      try {
        const a = await withTimeout(dns.resolve4(domain), MX_LOOKUP_TIMEOUT_MS);
        result = a.length > 0
          ? { status: "risky", reason: "No MX records — domain has A record fallback" }
      ...
      // Network failure / timeout — don't penalise the candidate, mark unchecked.
...
export async function validateEmail(email: string | null | undefined): Promise<EmailValidationResult> {
  ...
  try {
    ...
    logger.warn({ email: e, err: err instanceof Error ? err.message : String(err) }, "Email validation crashed");
