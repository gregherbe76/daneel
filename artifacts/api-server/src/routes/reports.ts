...
router.get("/reports/job/:jobId/latest/markdown", async (req, res) => {
  ...
  md.push("---");
  md.push("");
  md.push(`_Generated by ${branding.productName} · ${clientDisplayName} · ${new Date(generatedAt).toLocaleString()}_`);
...
router.get("/reports/job/:jobId/latest/pdf", async (req, res) => {
  ...
  // Brand mark: rounded square with checkmark + list lines
  const markX = 50;
  const markY = 28;
  const markSize = 36;
  ...
  doc.fill("#ffffff").fontSize(13).font("Helvetica-Bold")
    .text(branding.productName, markX + markSize + 10, markY + 11, { width: W - markSize - 10 });

  // Client logo (top-right of header if available)
  if (logoBuffer) {
    try {
      doc.image(logoBuffer, doc.page.width - 100, 12, { fit: [72, 36], align: "right" });
    } catch {
      // Skip logo if image format is unsupported
    }
  }
  ...
  doc.fill("#94a3b8").fontSize(10).font("Helvetica").text(
    `${job.location}  ·  ${job.seniority}  ·  ${reportTitle}`,
    50,
    106,
    { width: W }
  );
  ...
  doc.fill(MUTED).fontSize(8).font("Helvetica")
    .text(
      `${branding.productName}  |  Confidential — ${reportTitle}  |  ${new Date(generatedAt).toLocaleString()}`,
      50,
      doc.page.height - 22,
