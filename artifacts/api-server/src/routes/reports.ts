...
router.get("/reports/job/:jobId/latest/markdown", async (req, res) => {
  ...
  md.push("---");
  md.push("");
  md.push(`_Report generated by ${branding.productName} on ${new Date(generatedAt).toLocaleString()}_`);
...
router.get("/reports/job/:jobId/latest/pdf", async (req, res) => {
  ...
  doc.fill(MUTED).fontSize(8).font("Helvetica")
    .text(
      `${branding.productName}  |  Confidential — Hiring Manager Report  |  ${new Date(generatedAt).toLocaleString()}`,
      50,
      doc.page.height - 22,
