...
router.get("/reports/job/:jobId/latest/markdown", async (req, res) => {
  ...
  md.push(`# Client Shortlist Report — ${job.title}`);
  ...
  md.push("---");
  md.push("");
  md.push(`_Report generated by ShortlistPro — powered by Daneel — on ${new Date(generatedAt).toLocaleString()}_`);
...
router.get("/reports/job/:jobId/latest/pdf", async (req, res) => {
  ...
  doc.fill(MUTED).fontSize(8).font("Helvetica")
    .text(
      `ShortlistPro · Powered by Daneel  |  Confidential — Client Shortlist Report  |  ${new Date(generatedAt).toLocaleString()}`,
      50,
      doc.page.height - 22,
