...
} from "./marketplace/connect-cards";
...
/**
 * Minimal structural shape of a provider row that ScoutMarketplaceCard needs.
 * Kept narrower than `Provider` so callers in other files (notably the
 * marketplace catalog page) can pass their own `ProviderRecord` without an
 * unsafe cast — both shapes carry these fields.
 */
export type ScoutCardProvider = {
  id: number;
  name: string;
  type: string;
  enabled: boolean;
...
export function AdvancedProvidersSection() {
  ...
  return (
    <div className="space-y-10">
      ...
      <section>
        <div className="mb-4">
          <h2 className="text-base font-semibold text-foreground">A-Player integrations</h2>
          <p className="text-sm text-muted-foreground">
            One-click OAuth-style integrations with first-party A-Player agents.
            No copy-pasting API keys.
          </p>
        </div>
        <div className="space-y-3">
          {MARKETPLACE_CARDS.map((spec) => (
            <MarketplaceConnectCard
              key={spec.id}
              spec={spec}
